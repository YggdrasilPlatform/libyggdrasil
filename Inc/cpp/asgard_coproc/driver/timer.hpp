  ...
  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/**
  *  @file cpp/asgard_coproc/driver/timer.hpp
  *  @ingroup asgard_coproc
  *  @author Fabian Weber, Nikolaij Saegesser
  *  @brief Timer Channel abstraction implementation for Asgard
  */
...
namespace bsp::asg_coproc::drv {
	...
	/**
	 * @brief Timer channel implementation for Asgard
	...
	private:
		...
		bool hasEncoderModule() const noexcept{
			if ((Context->Instance == TIM1) ||		// All timer with a Encoder Module
					...
					(Context->Instance == TIM12) ||
					(Context->Instance == TIM15)) {
				return true;
			}
			return false;
		}
	...
	private:
		...
		u64 cntToNanoSeconds(Size cntValue) const noexcept {
			...
			if ((Context->Instance == TIM1) ||			// Get the timerFrequency before prescaler
					...
					(Context->Instance == TIM15) ||
					(Context->Instance == TIM16) ||
					(Context->Instance == TIM17)) {
				float pclk2 = static_cast<float>(HAL_RCC_GetPCLK2Freq()); 					// Get the timer clock before prescaler for APB2
				if ((RCC->TIMG2PRER & RCC_TIMG2PRER_TIMG2PRE) == 0) timerFrequency = pclk2;	// Timer frequency when APB2 prescaler = 1
				else timerFrequency = 2 * pclk2;											// Timer frequency when APB2 prescaler = 2

			}
			else {
				float pclk1 = static_cast<float>(HAL_RCC_GetPCLK1Freq()); 					// Get the timer clock before prescaler for APB1
				if ((RCC->TIMG1PRER & RCC_TIMG1PRER_TIMG1PRE) == 0) timerFrequency = pclk1;	// Timer frequency when APB1 prescaler = 1
	...
	struct Timer {
		...
		static u32 getPwmFrequency() {
			...
			if ((Context->Instance == TIM1) ||			// Get the timerFrequency before prescaler
					...
					(Context->Instance == TIM17)) {
				float pclk2 = static_cast<float>(HAL_RCC_GetPCLK2Freq()); 							// Get the timer clock before prescaler for APB2
				if ((RCC->TIMG2PRER & RCC_TIMG2PRER_TIMG2PRE) == 0) return (pclk2 / psc) / arr;		// Return the pwm frequency when APB2 prescaler = 1
				else return 2 *  (pclk2 / psc) / arr;												// Return the pwm frequency when APB2 prescaler = 2

			}
			else {
				float pclk1 = static_cast<float>(HAL_RCC_GetPCLK1Freq()); 							// Get the timer clock before prescaler for APB1
				if ((RCC->TIMG1PRER & RCC_TIMG1PRER_TIMG1PRE) == 0) return (pclk1 / psc) / arr;		// Return the pwm frequency when APB1 prescaler = 1
				else return 2 *  (pclk1 / psc) / arr;												// Return the pwm frequency when APB1 prescaler = 2
			}
		}
